...
class Urls
{
    ...
    {
        $wp = self::sanitize_title_with_dashes(str_replace("'", '-', self::remove_accents($title)));
        if (is_null(self::$urls)) {
            self::$urls = array();
        }
        if (in_array($wp, self::$urls)) {
            $i = 2;
            $ext = sprintf('-%d', $i);
            while (in_array($wp.$ext, self::$urls)) {
                ++$i;
                $ext = sprintf('-%d', $i);
            }
            $wp = $wp.$ext;
